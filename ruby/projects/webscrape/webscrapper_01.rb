...
def scrapper_port(port_url,country)
	...
	number_of_ports.each { |x| print_xml(country, x) }  
end

def print_xml(country, port)
		puts country
		open('vicky.xml', 'a') do |f|
			f << '<item type="com.oilspace.apps.aspect.items.Location">' + "\n" 
        #puts '<item type="com.oilspace.apps.aspect.items.Location">'
			f << '  <ivar name="aspect-edit-id" id="' + port + '">' + port + '</ivar> ' + "\n"
        #puts '  <ivar name="aspect-edit-id" id="' + port + '>#' + port + '</ivar> '
			f << '  <ivar name="aspect-edit-name">' + port + '</ivar> ' + "\n"
        #puts '  <ivar name="aspect-edit-name">' + port + '</ivar> '
			f << '  <ivar name="aspect-edit-pid" id="' + country + '">' + country + '</ivar>' + "\n"
