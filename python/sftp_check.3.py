...
x4 = read_config.get("logs", "sftp_log")

logging.basicConfig(format='[ %(asctime)s ]  %(message)s', level=logging.INFO, datefmt='%d.%m.%y %H:%M:%S', filename = x4)
...
def sftp_download(x1, x2, x3, content ):
    ...
    with pysftp.Connection(host=x1,username=x2,password=x3, cnopts=cnopts) as conn :
        logging.info("SFTP connected and proceed with download ")
        ...
        if os.path.exists(f1):
           ...
           #print("None")
           os.mkdir(f1)
           logging.info("Directory has been created {}".format(f1))
           conn.get_d(content , f1 , preserve_mtime=True)
           conn.close()
...
logging.info("Copy to destination started")
#os.system('rsync -avz {}/ {}'.format(d1, d2))
os.system("rsync -avz {}/ {}".format(d1, d2))
logging.info("Copy to destination completed")
